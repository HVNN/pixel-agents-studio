...
export class PixelAgentsViewProvider implements vscode.WebviewViewProvider {
  ...
  // All registered CLI providers, keyed by id
  ...
  resolveWebviewView(webviewView: vscode.WebviewView) {
    ...
    webviewView.webview.onDidReceiveMessage(async (message) => {
      if (message.type === 'openClaude' || message.type === 'openAgent') {
        const providerId = (message.providerId as string | undefined) ?? 'claude';
        const provider = this.providers.get(providerId) ?? claudeProvider;
        ...
        const availableProviders = Array.from(this.providers.values()).map((p) => ({
          id: p.id,
