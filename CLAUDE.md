...
# Watch mode (extension backend only — webview NOT included)
npm run watch
# After webview changes: npm run build:webview

# Mocked webview dev server (no VS Code needed, localhost:5173)
cd webview-ui && npm run dev
```

F5 for Extension Dev Host.

Testing:

- `npm test` — all unit/integration tests (webview + server); **build first** since server tests need compiled `dist/hooks/claude-hook.js`
- `npm run test:server` — server tests (Vitest)
- `npm run test:webview` — webview asset integration tests (Node test runner)
- `npm run e2e` — Playwright E2E tests (real VS Code instance; first run downloads ~200 MB VS Code into `.vscode-test/`)
- `npm run e2e:debug` — step-by-step debug mode

Other:

- `npm run lint` — ESLint across extension + server + webview
...
Defined in `eslint-rules/pixel-agents-rules.mjs`. Set to `error` — violations block the build.
