...
export async function launchNewTerminal(
  ...
  const displayName = provider?.displayName ?? TERMINAL_NAME_PREFIX;
  ...
  if (provider?.buildLaunchCommand) {
    const { command, args } = provider.buildLaunchCommand(sessionId, cwd, bypassPermissions);
    terminal.sendText([command, ...args].join(' '));
  } else {
    // Fallback: Claude Code default (keeps backward compat when no provider is passed)
  ...
  // Use provider's session dirs when available, fall back to Claude default resolution.
  const projectDir = provider?.getSessionDirs?.(cwd)?.[0] ?? getProjectDirPath(cwd);

  // Pre-register expected JSONL file so project scan won't treat it as a /clear file.
  // Providers without a sessionFilePattern don't produce JSONL — skip pre-registration.
  const sessionFilePattern = provider?.sessionFilePattern;
  const expectedFile = sessionFilePattern ? path.join(projectDir, `${sessionId}.jsonl`) : '';
  if (expectedFile) knownJsonlFiles.add(expectedFile);
